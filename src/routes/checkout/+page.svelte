<script lang="ts">
  ...
  let cart: any[] = $derived(cartStore?.cart || []);

  let savedAddresses = $state([] as string[]);
  let mousePos = $state({ x: 0, y: 0 });
  let step = $state<'form' | 'otp' | 'processing' | 'success'>('form');
  let otp = $state('');
  let otpError = $state('');

  let formData = $state({ name: '', email: '', phone: '', address: '' } as { name: string; email: string; phone: string; address: string; });

  let cartTotal = $derived(cart.reduce((total: number, item: any) => total + (item.price * item.quantity), 0));
...
function updateQuantity(id: string, size: string, color: string, delta: number) {
    if (!cartStore) return;
    let newCart = [...cartStore.cart];
    // Now checks for id, size AND color to ensure the correct variant is updated
    const index = newCart.findIndex(item => item.id === id && item.size === size && item.color === color);
    ...
    {:else}
      ...
      {#if step === 'form'}
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div class="lg:col-span-2 space-y-8">
            <form id="checkout-form" onsubmit={handleCheckout} class="bg-white/2 bg-linear-to-br from-white/5 to-transparent backdrop-blur-2xl border border-white/10 rounded-3xl p-8 space-y-6 shadow-[0_8px_32px_0_rgba(0,0,0,0.5)]">
              <h2 class="text-xl font-bold text-white border-b border-gray-800 pb-4">Contact Information</h2>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div><label for="checkout-name" class="block text-sm font-medium text-gray-400 mb-2">Full Name</label><input id="checkout-name" type="text" bind:value={formData.name} required class="w-full bg-gray-800/80 border border-gray-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#ff6b00] transition-colors" /></div>
                <div><label for="checkout-email" class="block text-sm font-medium text-gray-400 mb-2">Email</label><input id="checkout-email" type="email" bind:value={formData.email} required class="w-full bg-gray-800/80 border border-gray-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#ff6b00] transition-colors" /></div>
                <div class="md:col-span-2"><label for="checkout-phone" class="block text-sm font-medium text-gray-400 mb-2">Phone Number</label><input id="checkout-phone" type="tel" bind:value={formData.phone} required class="w-full bg-gray-800/80 border border-gray-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#ff6b00] transition-colors" /></div>
              </div>
                ...
                <div class="space-y-3 mb-6">
                  ...
                  {#each savedAddresses as addr}
                    <label class={`flex items-center p-4 border rounded-xl cursor-pointer transition-all ${formData.address === addr ? 'bg-[#ff6b00]/10 border-[#ff6b00]' : 'bg-gray-800/80 border-gray-700 hover:border-gray-500'}`}>
                      <input type="radio" name="saved_address" value={addr} checked={formData.address === addr} onchange={(e) => {
                        const target = e.target as HTMLInputElement | null;
                        if (target) formData.address = target.value;
                      }} class="w-4 h-4 text-[#ff6b00] bg-gray-900 border-gray-600 focus:ring-[#ff6b00]" />
              ...
              {/if}
              <div>
                <label for="checkout-address" class="block text-sm font-medium text-gray-400 mb-2">Confirm / Edit Address</label>
                <textarea id="checkout-address" bind:value={formData.address} required rows="3" class="w-full bg-gray-800/80 border border-gray-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#ff6b00] transition-colors" placeholder="Where should we deliver this?"></textarea>
              </div>
            </form>
          </div>

          <div class="lg:col-span-1">
            <div class="bg-white/2 bg-linear-to-br from-white/5 to-transparent backdrop-blur-2xl border border-white/10 rounded-3xl p-8 sticky top-36 shadow-[0_8px_32px_0_rgba(0,0,0,0.5)]">
                ...
                {#each cart as item}
                  <div class="flex justify-between items-start text-sm">
                    <div class="flex items-start space-x-4">
                      ...
                      <div class="w-16 h-16 bg-gray-800 rounded-lg overflow-hidden shrink-0 border border-gray-700">
                        <img 
                          src={item.images && item.images[item.color] ? item.images[item.color] : item.image} 
                          alt={item.name} 
                      ...
                      <div class="flex flex-col">
                        ...
                        <p class="text-gray-500 mb-2 text-xs">Size: {item.size} &bull; Color: {item.color}</p>
                        
                        <div class="flex items-center bg-gray-800/80 border border-gray-700 rounded-md w-max">
                          <button type="button" onclick={() => updateQuantity(item.id, item.size, item.color, -1)} class="px-3 py-1 text-gray-400 hover:text-[#ff6b00] hover:bg-gray-700 transition-colors rounded-l-md cursor-pointer">-</button>
                          
                          <span class="px-3 text-white text-xs font-bold w-8 text-center">{item.quantity}</span>
                          
                          <button type="button" onclick={() => updateQuantity(item.id, item.size, item.color, 1)} class="px-3 py-1 text-gray-400 hover:text-[#ff6b00] hover:bg-gray-700 transition-colors rounded-r-md cursor-pointer">+</button>
                        </div>
                      </div>
      ...
      {:else if step === 'otp'}
        <div class="max-w-md mx-auto mt-12">
          <div class="bg-white/2 bg-linear-to-br from-white/5 to-transparent backdrop-blur-2xl border border-white/10 rounded-3xl p-8 shadow-[0_8px_32px_0_rgba(0,0,0,0.5)] text-center">
            ...
            <form onsubmit={handleVerifyOTP} class="space-y-6">
              <div>
                <input type="text" maxlength="6" value={otp} oninput={(e) => {
                    const target = e.target as HTMLInputElement | null;
                    if (target) otp = target.value.replace(/\D/g, '');
                  }} class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-4 text-center text-2xl tracking-[0.5em] text-white font-mono focus:outline-none focus:border-[#ff6b00] transition-colors" placeholder="••••••" required />
      ...
      {:else if step === 'success'}
        <div class="max-w-md mx-auto mt-12">
          <div class="bg-white/2 bg-linear-to-br from-white/5 to-transparent backdrop-blur-2xl border border-green-500/30 rounded-3xl p-8 shadow-[0_8px_32px_0_rgba(34,197,94,0.15)] text-center relative overflow-hidden">
            <div class="absolute top-0 left-0 w-full h-1 bg-linear-to-r from-transparent via-green-500 to-transparent"></div>
